  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white py-8">
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-4xl font-bold text-yellow-400 mb-8">Starships</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((ship, idx) => (
            ...
            >
              <h3 className="text-xl font-bold mb-3 text-yellow-400">{ship.name}</h3>
              <div className="space-y-2 text-sm">
                <div>
                  <span className="text-gray-400">Model:</span>
                  <span className="ml-2">{ship.model}</span>
                </div>
                <div>
                  <span className="text-gray-400">Manufacturer:</span>
                  <span className="ml-2">{ship.manufacturer}</span>
                </div>
                <div>
                  <span className="text-gray-400">Class:</span>
                  <span className="ml-2">{ship.starship_class}</span>
                </div>
                <div>
                  <span className="text-gray-400">Cost:</span>
                  <span className="ml-2">{ship.cost_in_credits === 'unknown' ? 'Unknown' : `${parseInt(ship.cost_in_credits).toLocaleString()} credits`}</span>
                </div>
              </div>
            </div>
          ))}
